...
def process_rows(rows, course_code, course_name, year, save_dir, command='', merge=True, video_type='both'):
    ...
    def process_video(video_url, track_type, row):
        ...
        prev_filepath_mp4 = os.path.join(save_dir, f"{course_code}{course_name}{year}年{month}月{date}日第{days}周星期{day_chinese}第{jie-1}节-{track_type}.mp4")
        next_filepath_mp4 = os.path.join(save_dir, f"{course_code}{course_name}{year}年{month}月{date}日第{days}周星期{day_chinese}第{jie+1}节-{track_type}.mp4")
        
        # 也检查.ts格式的文件（向后兼容）
        prev_filepath_ts = os.path.join(save_dir, f"{course_code}{course_name}{year}年{month}月{date}日第{days}周星期{day_chinese}第{jie-1}节-{track_type}.ts")
        next_filepath_ts = os.path.join(save_dir, f"{course_code}{course_name}{year}年{month}月{date}日第{days}周星期{day_chinese}第{jie+1}节-{track_type}.ts")
